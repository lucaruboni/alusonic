...
function torresan_setup_pods(): void
{
    ...
    try {
        ...
        $pid = $pp ? ($pp['id'] ?? 0) : 0;
        if (! $pid) {
            $pid = $api->save_pod(['name'=>'page','label'=>'Pagine','type'=>'post_type','object'=>'page','storage'=>'meta']);
        }
        if (! $pid) { return; }
        ...
        $cid = $cp ? ($cp['id'] ?? 0) : 0;
        if (! $cid) {
            $cid = $api->save_pod(['name'=>'camera','label'=>'Models','type'=>'post_type','object'=>'camera','storage'=>'meta']);
        }
        if ($cid) {
            torresan_ensure_group($api, $cid, 'dettagli_modello', 'Model Details');
            $w = 0;
            torresan_ensure_field($api, ['pod_id'=>$cid,'group'=>'dettagli_modello','name'=>'model_category','label'=>'Categoria (etichetta)','description'=>'Es. "Basso · Custom Shop", "Chitarra · Hybrid", "Cabinet".','type'=>'text','weight'=>$w++]);
            torresan_ensure_field($api, ['pod_id'=>$cid,'group'=>'dettagli_modello','name'=>'model_type','label'=>'Tipo (per filtro)','type'=>'pick','pick_object'=>'custom-simple','pick_custom'=>"basso|Bassi\nchitarra|Chitarre\ncabinet|Cabinet",'pick_format_type'=>'single','pick_format_single'=>'dropdown','weight'=>$w++]);
        ...
        $eid = $ep ? ($ep['id'] ?? 0) : 0;
        if (! $eid) {
            $eid = $api->save_pod(['name'=>'esperienza','label'=>'Artists','type'=>'post_type','object'=>'esperienza','storage'=>'meta']);
        }
        if ($eid) {
            ...
            torresan_ensure_field($api, ['pod_id'=>$eid,'group'=>'dettagli_artista','name'=>'artist_quote','label'=>'Citazione','type'=>'paragraph','weight'=>$w++]);
